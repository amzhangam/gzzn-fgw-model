 ...
 */
public class BudgetOrganProjectListExpExcel extends JxlExcelCellUtil<PjbaseinfoPojo> {
...
//	
	 ...
	 * @return
	 */
	private WritableSheet writableSheetCell(PjbaseinfoPojo obj,
			Map<Integer,String> xmflMap,
			Map<Integer,String> organMap,int column,
			int row, int dataStartIndex, WritableSheet sheet) {
		try {
			sheet.addCell(expDataType(0,row + dataStartIndex, Integer.valueOf(row+1)));
			sheet.addCell(expDataType(1,row + dataStartIndex, obj.getXmfl()==null?"":xmflMap.get(obj.getXmfl())));
			String xmlxString = "";
			if(obj.getXmsbXmlx()!=null&&obj.getXmsbXmlx().getXmlxId()!=null&&obj.getXmsbXmlx().getXmlxId()>=2&&obj.getXmsbXmlx().getXmlxId()<=4){
				xmlxString = "新建项目";
			}
			else if(obj.getXmsbXmlx()!=null&&obj.getXmsbXmlx().getXmlxId()!=null&&obj.getXmsbXmlx().getXmlxId()>=7&&obj.getXmsbXmlx().getXmlxId()<=10){
				xmlxString = "新建项目";
			}
			else if(obj.getXmsbXmlx()!=null&&obj.getXmsbXmlx().getXmlxId()==6){
				xmlxString = "续建项目";
			}
			
			sheet.addCell(expDataType(2,row + dataStartIndex, xmlxString));
			sheet.addCell(expDataType(3,row + dataStartIndex, obj.getProjectcode()));
			sheet.addCell(expDataType(4,row + dataStartIndex, obj.getProjectname()));
			sheet.addCell(expDataType(5,row + dataStartIndex, (obj.getSysOrganizationByDeclareunitsid()!=null?obj.getSysOrganizationByDeclareunitsid().getOrganizationName():null)));
			sheet.addCell(expDataType(6,row + dataStartIndex, obj.getBudgetDeclareOrgan()));
			sheet.addCell(expDataType(7,row + dataStartIndex, obj.getDeclarerid()));
			sheet.addCell(expDataType(8,row + dataStartIndex, obj.getMobilePhone()));
			sheet.addCell(expDataType(9,row + dataStartIndex, obj.getXmjbqkms()));
			sheet.addCell(expDataType(10,row + dataStartIndex, (obj.getSysOrganizationByDirectorunitsId()!=null?obj.getSysOrganizationByDirectorunitsId().getOrganizationName():null)));
			sheet.addCell(expDataType(11,row + dataStartIndex, obj.getBudgetDirectorOrgan()));
			sheet.addCell(expDataType(12,row + dataStartIndex, obj.getPjauditmind()));
			sheet.addCell(expDataType(13,row + dataStartIndex, obj.getPjinvestcitySh()));
			sheet.addCell(expDataType(14,row + dataStartIndex, obj.getPlaninvestcitySh()));
			sheet.addCell(expDataType(15,row + dataStartIndex, obj.getPjauditdept()));
		} catch (RowsExceededException e) {
			e.printStackTrace();
		} catch (WriteException e) {
			e.printStackTrace();
	...
	public InputStream expExcelTempFile(SysUser sysUser, List<PjbaseinfoPojo> expData,
			Map<Integer,String> xmflMap,Map<Integer,String> organMap, String tempFilePath,
			int dataStartIndex) {
...
//	    	sheet.addCell(expDataType(3, 1, sysUser.getSysOrganization()==null?"":sysUser.getSysOrganization().getOrganizationName()));//填报单位（盖章）：
	    	...
	    	for (int i = 0; i < expData.size(); i++) {
	    		...
	    		writableSheetCell(obj, xmflMap,organMap, 0, i, dataStartIndex, sheet);
	    	}
