...
<SCRIPT LANGUAGE="JavaScript">
	...
	function infoToDelete(objTD) {
		...
		$.ajax({
			type : "POST",
			url : "${ctx }/resource/codeBasSourcedeptDelete.json",
			data : {
				ids : objTD
			},
...
<script type="text/javascript">	$(function(){
	...
	<form action="${ctx }/resource/codeBasSourcedeptQuery.ac" method="post">
		<table width="100%" border="0" cellpadding="0" cellspacing="0"
			...
			<tr>
				...
				<td width="154" class="c4">数据来源单位查询</td>
				...
				<td width="80%" background="../resources/images/bg_k2.gif">
					<div align="right">
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							<tr>
								<td width="66%">
								<strong>数据来源单位名称:</strong> <input type="text"
									name="dto.sourcedeptname" value="${dto.sourcedeptname }"  class="text">&nbsp;&nbsp;
								</td>
								<td>&nbsp;<input name="new4" type="button" class="btn" id='queryBtn'
									value="查询"> &nbsp;<input name="new4" type="button"
									class="btn" value="新增"
									onClick="javascrip:window.location.href='codeBasSourcedeptInit.ac'">
				...
				<c:forEach items="${page.list}" var="obj" varStatus="status">
					<tr>
						<td><input type="checkbox" name='checkedId'
							value='${obj.sourcedeptid}' id='checkedId' />
						</td>
						<td>${status.count }</td>
						<td>${obj.sourcedeptname}</td>
						<td>${obj.sourcedeptcode}</td>
					<!-- 	<td>${obj.parentSourcedeptname}</td> -->
						<td><a
							href='${ctx }/resource/codeBasSourcedeptInit.ac?sourcedeptid=${obj.sourcedeptid}'
							style="cursor: pointer; text-decoration: underline;">修改</a>&nbsp;&nbsp;||&nbsp;&nbsp;
							<a onclick="javascript:deleteOnClick(${obj.sourcedeptid});"
							style='cursor: pointer;'>删除</a>
						</td>
					</tr>
				</c:forEach>
				<tr>
					<td colspan="5" class="line2" style="text-align: right;"><%@include
