...
<SCRIPT LANGUAGE="JavaScript">
	...
	function infoToDelete(objTD) {
		$.ajax({
			type : "POST",
			url : "${ctx }/resource/ersCmnctDelete.json",
			data : {
				ids : objTD
			},
	...
	function showResponse(data) {
		if (data.success) {
			...
			window.location.href = "${ctx }/resource/ersCmnctExpertQuery.ac";
		} else {
			parent.mac.alert('失败');
		}
	}
...
<body style="overflow: hidden">
	<form action="${ctx }/resource/ersCmnctQuery.ac" method="post">
		<table width="100%" border="0" cellpadding="0" cellspacing="0"
			style="word-break: keep-all">
			<tr>
				<td width="13"><img src="../resources/images/k0.gif" width="13"
					height="37"></td>
				<td width="154" class="c4">通信保障信息</td>
				...
				<td width="998" background="../resources/images/bg_k2.gif">
					<div align="right">
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							<tr>
								<td width="76%" align="left"><strong>类别名称:</strong><input type="hidden"
									class="text" name="dto.classifyId" value="${dto.classifyId }"
									id='classifyId' /> <input type="text" class="text"
									name="dto.classifyName" id="classifyName"
									value="${dto.classifyName}" readonly="readonly" />&nbsp;&nbsp;<strong>名称:</strong> <input type="text" class="text"
									name="dto.name" id="name"
									value="${dto.name}"  /></td>
								<td>&nbsp;<input name="new4" type="button" class="btn" id='queryBtn'
									value="查询"> &nbsp;<input name="new4" type="button" 
									class="btn" value="新增"
									onClick="javascrip:window.location.href='ersCmnctInit.ac'">
				...
				<c:forEach items="${page.list}" var="obj" varStatus="status">
					<tr>
						<td><input type="checkbox" name='checkedId'
							value='${obj.communicationid}' id='checkedId' /></td>
						<td>${status.count }</td>
						<td>${obj.nucode}</td>
						<td>${obj.name}</td>
						...
						<td>${obj.notes}</td>
						<td><a
							href='${ctx }/resource/ersCmnctInit.ac?communicationid=${obj.communicationid}'
							style="cursor: pointer; text-decoration: underline;">修改</a>&nbsp;&nbsp;||&nbsp;&nbsp;
							<a onclick="javascript:deleteOnClick(${obj.communicationid});"
