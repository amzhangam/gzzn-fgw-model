...
function createDoc(){
	OFFICE_CONTROL_OBJ=document.all("NTKO_OCX");
...
function intializePageExcel(fileUrl)
{
	//alert(fileUrl);
	OFFICE_CONTROL_OBJ=document.all("NTKO_OCX");
	//initCustomMenus();	
	NTKO_OCX_OpenExcel(fileUrl);
}
function NTKO_OCX_OpenExcel(fileUrl)
{
	OFFICE_CONTROL_OBJ.BeginOpenFromURL(fileUrl,false,false,"Excel.Sheet");
}
function onPageClose(url)
{
/*
...
function saveFileToUrl(myUrl){
	var result = OFFICE_CONTROL_OBJ.saveToURL(myUrl,//提交到的url地址
		"upload",//文件域的id，类似<input type=file name=upload 中的name
...
function saveFile(myUrl,parameter,filename){
	var result = OFFICE_CONTROL_OBJ.saveToURL(myUrl,//提交到的url地址
		"upload",//文件域的id，类似<input type=file name=upload 中的name
		parameter,          //与控件一起提交的参数如："p1=a&p2=b&p3=c"
		filename,    //上传文件的名称，类似<input type=file 的value
		0    //与控件一起提交的表单id，也可以是form的序列号，这里应该是0.
		);
	OFFICE_CONTROL_OBJ.activeDocument.saved=true;
...
{
	alert(strHeader);
	OFFICE_CONTROL_OBJ.ActiveDocument.Application.Selection.TypeText(strHeader);
	alert(strHeader);
	var i,cNum = 30;
	var lineStr = "";
	try
...
function showInput(obj){
	//var obj = window.dialogArguments;//取得父窗口传过来的参数
	//alert(obj.title);
	//修改退文表模板，写入参数值
	
	OFFICE_CONTROL_OBJ.SetBookmarkValue("标题",obj.title);
	OFFICE_CONTROL_OBJ.SetBookmarkValue("来文单位",obj.comeUnit);
	OFFICE_CONTROL_OBJ.SetBookmarkValue("文件标题或编号",obj.doctitleorcode);
	OFFICE_CONTROL_OBJ.SetBookmarkValue("收文日期",obj.receiveDate);
	OFFICE_CONTROL_OBJ.SetBookmarkValue("紧急程度",obj.emergency);
	OFFICE_CONTROL_OBJ.SetBookmarkValue("秘密等级",obj.secret);
	OFFICE_CONTROL_OBJ.SetBookmarkValue("经办人",obj.handler);
	OFFICE_CONTROL_OBJ.SetBookmarkValue("审核人",obj.checker);
	OFFICE_CONTROL_OBJ.SetBookmarkValue("联系电话",obj.tel);
	OFFICE_CONTROL_OBJ.SetBookmarkValue("制表日期",obj.createDate);
	for(var i=0;i<9;i++){
		OFFICE_CONTROL_OBJ.SetBookmarkValue("标识" + (i+1),obj.checkArray[i]);
		OFFICE_CONTROL_OBJ.SetBookmarkValue("备注" + (i+1),obj.remarkArray[i]);
	}
	//Josen modify 2010-4-6
	OFFICE_CONTROL_OBJ.SetReadOnly(false,"");//设置文档为只读
	OFFICE_CONTROL_OBJ.activeDocument.saved=false;//设置文档未被修改标志为true
	
	/*
	OFFICE_CONTROL_OBJ.SetBookmarkValue("标识1","√");
	OFFICE_CONTROL_OBJ.SetBookmarkValue("标识2","√");
	OFFICE_CONTROL_OBJ.SetBookmarkValue("标识3","");
	OFFICE_CONTROL_OBJ.SetBookmarkValue("标识4","");
	OFFICE_CONTROL_OBJ.SetBookmarkValue("标识5","");
	OFFICE_CONTROL_OBJ.SetBookmarkValue("标识6","");
	OFFICE_CONTROL_OBJ.SetBookmarkValue("标识7","");
	OFFICE_CONTROL_OBJ.SetBookmarkValue("标识8","");
	OFFICE_CONTROL_OBJ.SetBookmarkValue("标识9","");
	OFFICE_CONTROL_OBJ.SetBookmarkValue("备注1","无备注");
	OFFICE_CONTROL_OBJ.SetBookmarkValue("备注2","无备注");
	OFFICE_CONTROL_OBJ.SetBookmarkValue("备注3","");
	OFFICE_CONTROL_OBJ.SetBookmarkValue("备注4","");
	OFFICE_CONTROL_OBJ.SetBookmarkValue("备注5","");
	OFFICE_CONTROL_OBJ.SetBookmarkValue("备注6","");
	OFFICE_CONTROL_OBJ.SetBookmarkValue("备注7","");
	OFFICE_CONTROL_OBJ.SetBookmarkValue("备注8","");
	OFFICE_CONTROL_OBJ.SetBookmarkValue("备注9","");
	*/
}
//设置对话框返回值，并关闭对话框
function back(returnValue){
	window.returnValue = returnValue;
	window.close();
}

//初始化显示新建反馈表表头
function initFeedback(isNewFile){
	if(isNewFile == "1"){
		var obj = window.dialogArguments;//取得父窗口传过来的参数
		//alert(obj);
		OFFICE_CONTROL_OBJ.SetRangeValue("sheet1","A1",obj);
	}
}

//打印发文表
function printTable(){
	OFFICE_CONTROL_OBJ.PrintOut(false);
}
