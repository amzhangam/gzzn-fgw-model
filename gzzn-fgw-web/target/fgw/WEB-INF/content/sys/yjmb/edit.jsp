...
<head>
<title>
<c:if test="${not empty obj.yjmbId}" var="result">
	编辑意见模板信息
</c:if>
<c:if test="${!result}">
	新增意见模板信息
...
//<!--
	   $(document).ready(function() {
			...
			$("#backBtn").click(function() {
				window.location = "${ctx}/sys/yjmb/list.ac";
			});
	...
	function checkRepeat(){
		$.post(
		    	"${ctx}/sys/yjmb/checkRepeat.json",
		    	{"mbmc":$("#mbmc").val(),"id":$("#yjmbId").val()},
		    	function(data){
		    		var json = $.parseJSON(data);
					if(json.flag){
						$("#editForm").attr("action","${ctx}/sys/yjmb/save.ac");
						$("#editForm").submit();
					}
					else{
						alert(json.msg);
	...
	<form id="editForm" action="${ctx}/sys/yjmb/save.ac" method="post">
		<input type="hidden" name="obj.yjmbId" id="yjmbId" value="${obj.yjmbId}"/>
		...
		<div class="editDiv">
			<table class="editTab" id="editTab">
				<tr>
					<th>意见模板名称<font class="msg">*</font></th>
					<td>
