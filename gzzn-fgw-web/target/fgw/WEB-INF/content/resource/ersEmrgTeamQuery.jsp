...
<SCRIPT LANGUAGE="JavaScript">
	...
	function infoToDelete(objTD) {
		$.ajax({
			type : "POST",
			url : "${ctx }/resource/ersEmrgTeamDelete.json",
			data : {
				ids : objTD
			},
...
<body style="overflow: hidden">
	...
	<form action="${ctx }/resource/ersEmrgTeamQuery.ac" method="post">
		<table width="100%" border="0" cellpadding="0" cellspacing="0"
			style="word-break: keep-all">
			<tr>
				<td width="13"><img src="../resources/images/k0.gif" width="13"
					height="37"></td>
				<td width="154" class="c4">应急救援队伍</td>
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							<tr>
								<td width="66%" align="left"><strong>类别名称:</strong> <input type="hidden"
									...
									class="text" name="dto.name" value="${dto.name }"
									id='name' /> 
									</td>
								<td>&nbsp;<input name="new4" type="button"   id='queryBtn' class="btn"
									value="查询"> &nbsp;<input name="new4" type="button"
									class="btn" value="新增"
									onClick="javascrip:window.location.href='ersEmrgTeamInit.ac'">
				...
				<c:forEach items="${page.list}" var="obj" varStatus="status">
					<tr>
						<td><input type="checkbox" name='checkedId'
							value='${obj.teamid}' id='checkedId' /></td>
						<td>${status.count }</td>
						<td>${obj.name}</td>
						<td>${obj.nucode}</td>
						...
						<td>${obj.foundeddate}</td>
						<td><a
							href='${ctx }/resource/ersEmrgTeamInit.ac?teamid=${obj.teamid}'
