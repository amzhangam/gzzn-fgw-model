...
<SCRIPT LANGUAGE="JavaScript">
	...
	function infoToDelete(objTD) {
		$.ajax({
			type : "POST",
			url : "${ctx }/resource/ersEmrgOrgDelete.json",
			data : {
				ids : objTD
			},
...
<body style="overflow: hidden">
	...
	<form action="${ctx }/resource/ersEmrgOrgQuery.ac" method="post">
		<table width="100%" border="0" cellpadding="0" cellspacing="0"
			style="word-break: keep-all">
			<tr>
				<td width="13"><img src="../resources/images/k0.gif" width="13"
					height="37"></td>
				<td width="154" class="c4">应急机构信息</td>
				...
				<td width="79%" background="../resources/images/bg_k2.gif">
					<div align="right">
						...
						value="${dto.classifyName}" readonly="readonly" id="classifyName" />&nbsp;&nbsp; <strong>名称:</strong> <input type="text"
									name="dto.orgname" class="text" size="10"
									value='${dto.orgname }'></td>
								<td>&nbsp;<input name="new4" type="button" id='queryBtn' class="btn"
									value="查询"> &nbsp;<input name="new4" type="button"
									class="btn" value="新增"
									onClick="javascrip:window.location.href='ersEmrgOrgInit.ac'">
				...
				<c:forEach items="${page.list}" var="obj" varStatus="status">
					<tr>
						<td><input type="checkbox" name='checkedId'
							value='${obj.orgid}' id='checkedId' /></td>
						<td>${status.count }</td>
						<td>${obj.nucode}</td>
						<td>${obj.orgname}</td>
						<td>${obj.classifyName }</td>
						<td>${obj.orgduty}</td>
						<td>${obj.firstleader}</td>
						<td>${obj.dutytel}</td>
						<td><a
							href='${ctx }/resource/ersEmrgOrgInit.ac?orgid=${obj.orgid}'
							style="cursor: pointer; text-decoration: underline;">修改</a>&nbsp;&nbsp;||&nbsp;&nbsp;
							<a onclick="javascript:deleteOnClick(${obj.orgid});"
