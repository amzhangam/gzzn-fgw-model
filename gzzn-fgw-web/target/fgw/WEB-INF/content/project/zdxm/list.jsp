...
<!--
	$(document).ready(function() {
		...
		$("#cancelZdxm").click(function(){
			...
			if(ids.length == 0){
				mac.alert("请选择要取消重大项目的记录");
				return;
			}
			ids = ids.substring(1);
			//alert(ids);
			mac.confirm('<p>确认要取消重大项目设置吗?</p>', function(){
				window.location = "${ctx}/project/zdxm/processGreatProject.ac?id=" + ids + "&flag="+0;
			}, null);
		});
...
//-->
	...
	//function setZdxm(id){
	//	mac.confirm('<p>确认要设置重大项目吗?</p>', function(){
	//		window.location = "${ctx}/project/zdxm/processGreatProject.ac?id=" + id + "&flag="+1;
	//	}, null);
	//}
	//
	//function cancelZdxm(id){
	//	mac.confirm('<p>确认要取消重大项目设置吗?</p>', function(){
	//		window.location = "${ctx}/project/zdxm/processGreatProject.ac?id=" + id + "&flag="+0;
	//	}, null);
	//}
	function setZdxm(ids){
			var url = "${ctx}/project/zdxm/zdxm.json?id=" + ids;
			var w = 500;     //宽度
			 var h = 310;   //高度
	...
	function plthxm(ids){
		mac.confirm('<p><font size="3" color="blue">确实要退回所选项目吗?</font></p>', function(){
			mac.confirm('<p><font size="3" color="red">事关重大，请再次确认要退回所选项目吗？</font></p>', function(){
				$.post(
						"${ctx}/project/zdxm/returnProject.json",
						{"id":ids},
						function(data){
					var json = $.parseJSON(data);
					if(json.flag){
						window.location = "${ctx}/project/zdxm/list.ac";
						alert(json.msg);
					}else{
						alert(json.msg);
					}
	...
	function viewZtjd(ids){
		var url = "${ctx}/project/zdxm/ztjd.json?id=" + ids;
		var w = window.open (url, 'newwindow6', 'left=0,top=0,width='+ (screen.availWidth - 10) +',height='+ (screen.availHeight-50) +',toolbar=no, menubar=no, scrollbars=yes,resizable=yes,location=no, status=no');
		w.focus();
	}
	
	function pjauditmind(projectId){
		var url = "${ctx}/project/ysdwxm/pjauditmind.json?id="+projectId;
		var pjauditmind = window.open (url, 'newwindow16', 'left=0,top=0,width='+ (screen.availWidth - 10) +',height='+ (screen.availHeight-50) +',toolbar=no, menubar=no, scrollbars=yes,resizable=yes,location=no, status=no');
		pjauditmind.focus();
		
	}
	
	//function setXmcblb(projectId){
	//	var url = "${ctx}/project/zdxm/xmcblb.json?id="+projectId;
	//	window.location.href = url;
	//}
	function setXmcblb(ids){
		var url = "${ctx}/project/zdxm/xmcblb.json?id=" + ids + "&pageFlag=1";
		var w = 500;     //宽度
		 var h = 310;   //高度
		 var t = (screen.height-h)/2; //离顶部距离
		 var l = (screen.width-w)/2; //离左边距离
	...
	<form action="${ctx}/project/zdxm/list.ac" method="post"  enctype="multipart/form-data" id="zdxm">
		...
		<table style="width:100%;height:300">
			<tr>
				...
				<td class="tbHeadTitle" id="headTitle">已通过审核项目</td>
				<td class="tbHeadBg" style="width: 5%;"><div class="tbHeadMid"></div></td>
				<td class="tbHeadBg" style="width: 79%;height:300">
					 <div>
					    <span style="float: right;">  
					  		<input type="button" class="btn" id="exportBtn" value="导出项目汇总表" onclick="exportexcel('${ctx}/project/zdxm/expExcel.ac','tabList',2);"/>          
			            	 <input type="button" class="btn" id="setxmcblb" value="批量设置项目储备类别"/>
					    	<c:if test="${objectMap['XMSB_XM_ZDXMGL_SET']}">
			            	 <input type="button" class="btn" id="setZdxm" value="批量设置重大项目"/>
			            	</c:if>
			            	<c:if test="${objectMap['XMSB_XM_ZDXMGL_PLTHXM']}">
			            	 <input type="button" class="btn" id="plthxm" value="批量退回项目"/>
			            	</c:if>
					    	<c:if test="${objectMap['XMSB_XM_ZDXMGL_VIEW']}">
			            	 <!--input type="button" class="btn" id="ztjd" value="查看总体进度"/-->
			            	</c:if>
		               	</span>   
		             </div>
				</td>
		...
		<thead>
			...
			<tr>
				...
				<td>
					...
					<a href="javascript:setXmcblb(${obj.projectid})">${obj.pjstatus>1&&obj.pjstatus!=12?'&nbsp;&nbsp;设置项目储备类别':''}</a>
					<c:if test="${objectMap['XMSB_XM_ZDXMGL_ZTJD']}">
					<!--a href="javascript:viewZtjd(${obj.projectid})">${obj.sfzdxm==1?'&nbsp;&nbsp;查看总体进度':''}</a-->
					</c:if>
					<c:if test="${objectMap['XMSB_XM_ZDXMGL_SET']}">
						<!--a href="javascript:setZdxm(${obj.projectid})">设置重大项目</a-->
					</c:if>
					<%
					...
					%>	
								<c:if test="${objectMap['XMSB_XM_DCLXMGL_XMPS']}">
									<a href="javascript:toxmps(${obj.projectid})"></a>${obj.pjstatus==10?'委托投资评审':''}
								</c:if>
					<%									
							}
						}
